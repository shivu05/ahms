...
class SHV_Controller extends MX_Controller
{
    ...
    function get_first_last_day_of_month($month = "")
    {
        if (!empty($month) && $month != '') {
            $query = "SELECT 
            DATE_ADD(DATE_ADD(LAST_DAY(STR_TO_DATE('2021-March-01','%Y-%M-%d')),
                    INTERVAL 1 DAY),
                INTERVAL - 1 MONTH) AS first_day,LAST_DAY(STR_TO_DATE('2021-March-01','%Y-%M-%d')) last_day";
