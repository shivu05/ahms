...
class Autoclave extends SHV_Controller {
    ...
    function index() {
        ...
        $data['top_form'] = modules::run('common_methods/common_methods/date_dept_selection_form', 'print-autoclave-list', true, false);
    ...
    function fetch_list() {
        $this->load->model('register/autoclave_register');
        $input_array = array();
        ...
        $data = $this->autoclave_register->get_data($input_array);
    ...
    function export_data() {
        ...
        $result = $this->autoclave_register->get_data($input_array, true);
        $this->layout->data = $result;
        $html = $this->layout->render(array('view' => 'register/autoclave/print_grid'), true);
        $title = array(
            'report_title' => 'AUTOCLAVE REGISTER',
        ...
        generate_pdf($html, 'L', $title, 'AUTOCLAVE_REGISTER_' . $current_date, true, true, 'D');
        exit;
    }
    ...
    function store_data() {
        ...
        $is_inserted = $this->autoclave_register->store($post_values);
        if ($is_inserted) {
