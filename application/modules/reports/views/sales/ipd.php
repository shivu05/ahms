...
<div class="modal fade" id="ipd_pharma_edit_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h5 class="modal-title" id="ipd_pharma_modal_label">Update pharmacy details</h5>
            </div>
            <div class="modal-body" id="ipd_pharma_modal_body">
                <form action="" name="ipd_pharma_form" id="ipd_pharma_form" method="POST">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#patient_statistics').on('click', '#ipd_pharma_table .product_edit', function () {
            var id = $(this).data('id');
            var prod_name = $(this).data('prod_name');
            var prod_qty = $(this).data('prod_qty');

            $('#ipd_pharma_edit_modal #ipd_pharma_form #product').val(prod_name);
            $('#ipd_pharma_edit_modal #ipd_pharma_form #qty').val(prod_qty);
            $('#ipd_pharma_edit_modal #ipd_pharma_form #id').val(id);
            $('#ipd_pharma_edit_modal').modal('show');
            $('#ipd_pharma_edit_modal #ipd_pharma_form').validate({
                rules: {
                    product: {
                        required: true
                    },
        ...
        $('#ipd_pharma_edit_modal').on('click', '#btn-update', function () {
            var form_data = $('#ipd_pharma_edit_modal #ipd_pharma_form').serializeArray();
            if ($('#ipd_pharma_edit_modal #ipd_pharma_form').valid()) {
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        $('#ipd_pharma_edit_modal').modal('hide');
                    },
                    error: function (err) {
                    }
                });
        ...
        $('#patient_statistics').on('click', '#ipd_pharma_table .product_delete', function () {
            var id = $(this).data('id');
