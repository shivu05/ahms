...
class Treatment extends SHV_Controller {
    ...
    public function __construct() {
        ...
        $this->layout->title = "Archived OPD List";
    ...
    function get_patients_for_treatment() {
        ...
        if (empty($input_array['arch_year']) || trim($input_array['arch_year']) == '') {
            ...
            $data = $this->treatment_model->get_patients($db, $input_array);
...
//            $this->treatment_model->add_ipd_treatment_data($ipd_treatment);
    ...
    function print_case_sheet($opd = null, $treat_id = null, $date = '') {
        if ($opd && $date) {
            $this->load->model('custom_db');
            $db_name = base64_decode($date);
            $db = $this->custom_db->getdatabase($db_name);
            $treat_data = $this->treatment_model->get_patient_treatment($db,$opd, $treat_id);
    ...
    function export() {
        ...
        unset($post_values['arch_year']);
        if ($type == 'cs') {
            try {
                $this->print_opd_casesheets($db,$post_values);
            } catch (Exception $e) {
    ...
    function print_opd_casesheets($db,$post_values) {
        ini_set("memory_limit", "-1");
        $data = $this->treatment_model->get_patients($db,$post_values, true);
        $this->load->helper('mpdf');
        if (!empty($data)) {
            $main_content = '';
            $n = count($data['data']);
            ...
            foreach ($data['data'] as $row) {
                $i++;
                $treat_data = $this->treatment_model->get_patient_treatment($db,$row['OpdNo'], $row['ID']);
                $config = $db->get('config');
                $config = $config->row_array();
                $header = '<table width="100%" style="border:1px red solid"><tr>
                    <td width="10%"><img src="' . base_url('assets/your_logo.png') . '" width="80" height="80" alt="logo"></td>
