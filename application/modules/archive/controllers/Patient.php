...
//        }
    ...
    function ipd_list() {
        ...
        $this->layout->navTitleFlag = false;
        $this->layout->navTitle = "Archived IPD patients";
        $this->layout->navDescr = "In Patient Department";
        $this->layout->title = "Archived IPD";
    ...
    function get_ipd_patients_list() {
        $this->load->model('archive/patient/ipd_model');
        $input_array = array();
        foreach ($this->input->post('search_form') as $search_data) {
            $input_array[$search_data['name']] = $search_data['value'];
        }
        if (empty($input_array['arch_year']) || trim($input_array['arch_year']) == '') {
            ...
            $input_array['length'] = $this->input->post('length');
            $input_array['order'] = $this->input->post('order');
            $data = $this->ipd_model->get_patients($db, $input_array);
            $response = array("recordsTotal" => $data['total_rows'], "recordsFiltered" => $data['found_rows'], 'data' => $data['data']);
            echo json_encode($response);
        }
    }
    ...
    function print_ipd_case_sheet($ipd = null, $date = '') {
        if ($ipd && $date) {
            ...
            $db = $this->custom_db->getdatabase($db_name);
            $opd_data = $this->treatment_model->get_opd_by_ipd($db,$ipd);
            $opd = $opd_data['OpdNo'];
            $patient_details = $this->treatment_model->get_ipd_patient_basic_details($db,$opd, $ipd);
            $data['treatment_details'] = $this->treatment_model->get_ipd_treatment_history($db,$ipd);
