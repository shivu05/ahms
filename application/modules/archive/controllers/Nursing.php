...
class Nursing extends SHV_Controller {
    ...
    function index() {
        ...
        $data['top_form'] = modules::run('common_methods/common_methods/date_dept_selection_form', 'archive/nursing/get_nursing_report_print', false, false, true);
        $this->layout->data = $data;
        $this->layout->render();
    }

    function get_nursing_report() {
        $data['title'] = "Archived NURSING REGISTER";
        ...
        $data["patient"] = $this->nursing_model->get_nursing_ipd_indent($db, $start_date, $end_date, $dept);
        $this->load->view('archive/nursing/nursing_report_ajax', $data);
    }
    ...
    function get_nursing_report_print() {
        ...
        generate_pdf($content, 'L', $title, 'ipd_nursing_report_' . $date, true, true, 'I');
        exit;
    }

    function nursing_indent_report() {
        $this->layout->title = "Archived Nursing Indent Report";
        $data['dept_list'] = $this->get_department_list('array');
        $data['top_form'] = modules::run('common_methods/common_methods/date_dept_selection_form', 'archive/nursing/get_nursing_indent_report_print', false, false, true);
        $this->layout->data = $data;
        $this->layout->render(array('view' => 'archive/nursing/nursing_indent_report'));
    }
    ...
    function get_nursing_indent_report() {
        ...
        $db_name = base64_decode($this->input->post('arch_year'));
        $db = $this->custom_db->getdatabase($db_name);
        $data["patient"] = $this->nursing_model->get_nursing_indent($db, $start_date, $end_date, $dept);
        $this->load->view('archive/nursing/nursing_indent_report_ajax', $data);
    }
    ...
    function get_nursing_indent_report_print() {
        ...
        $date = format_date($start_date);
        generate_pdf($content, 'L', $title, 'nursing_indent_report_' . $date, true, true, 'I');
        exit;
    }
