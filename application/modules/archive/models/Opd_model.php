...
/**
 * Description of archived opd_model
 *
 * @author Shivaraj
 */
class Opd_model extends CI_Model {
    ...
    function get_opd_patients($db = null, $conditions, $export_flag = FALSE) {
        $return = array();
        if (empty($db)) {
            return false;
        }
        ...
        $return['total_rows'] = $db->query('SELECT * FROM treatmentdata t JOIN patientdata p ON t.OpdNo=p.OpdNo')->num_rows();
        return $return;
    }
