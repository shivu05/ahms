...
class treatment_model extends CI_Model {
    ...
    function get_ipd_patient_basic_details($db,$opd, $ipd) {
        $db->from('patientdata p');
        $db->join('inpatientdetails i', 'p.OpdNo=i.OpdNo');
        $db->where('p.OpdNo', $opd);
        $db->where('i.IpNo', $ipd);
    ...
    public function get_opd_by_ipd($db,$ipd_no) {
        $db->select('OpdNo');
        return $db->get_where('inpatientdetails', array('IpNo' => $ipd_no))->row_array();
    }

    function get_ipd_treatment_history($db,$ipd) {
        $where = array(
            'ipdno' => $ipd
        );
        return $db->get_where('ipdtreatment', $where)->result_array();
    }
