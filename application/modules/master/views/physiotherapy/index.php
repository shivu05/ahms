<div class="row">
    <div class="col-12">
        <div class="box box-primary">
            <div class="box-header with-border"><h3 class="box-title"><i class="fa fa-list"></i> Physiotherapy reference:</h3>
            ...
            <div class="box-body">
                <table class="table table-bordered dataTable" id="physio_grid">
                    ...
                    <tbody>
                        <?php
                        if (!empty($physio_list)) {
                            $i = 0;
                            foreach ($physio_list as $row) {
                                $tr = '<tr>';
                                $tr .= '<td>' . ++$i . '</td>';
                                $tr .= '<td>' . $row['name'] . '</td>';
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="physio_add_modal_box_label">Add new Physiotherapy</h4>
            </div>
            <div class="modal-body" id="lab_modal_body">
                <form action="" name="test_form" id="test_form" method="POST">
                    <div class="row">
                        <div class="form-group col-md-6 col-sm-12">
                            <label class="control-label">Physiotherapy name:</label>
                            <input type="hidden" name="physi_id" id="physi_id"/>
                            <input class="form-control required" type="text" placeholder="Physiotherapy name" name="physi_name" id="physi_name" required="required" autocomplete="off">
                        </div>
                    </div>
                </form>
            </div>
    ...
    $(document).ready(function () {
        ...
        $('#physio_add_modal_box').on('click', '#btn-ok', function () {
            if ($('#test_form').valid()) {
                ...
                $.ajax({
                    url: base_url + 'add-physiotherapy',
                    type: 'POST',
                    dataType: 'json',
                    data: form_data,
        ...
        $('#physio_grid').on('click', '.delete_btn', function () {
            ...
            if (confirm('Are you sure want to delete?')) {
                $.ajax({
                    url: base_url + 'delete-physiotherapy',
                    type: 'POST',
                    dataType: 'json',
                    data: {physi_id: id},
                    success: function (res) {
