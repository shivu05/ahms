...
<div style="background-color: #e6f2ff; border: 1px solid #000; border-radius: 8px; padding: 5px;">
    ...
    function register_death()
    {
        $this->output->set_content_type('application/json');
        $this->load->model('patient/death_register_model');

        $ipd = (int) $this->input->post('death_ipd');
        if ($ipd <= 0) {
            echo json_encode(array('status' => false, 'message' => 'Invalid IPD number.'));
            return;
        }

        $post_data = array(
            'death_date' => $this->input->post('death_date'),
            'death_time' => $this->input->post('death_time'),
            'certifying_doctor' => $this->input->post('certifying_doctor'),
            'verified_by' => $this->input->post('verified_by'),
            'final_diagnosis' => $this->input->post('final_diagnosis'),
            'immediate_cause' => $this->input->post('immediate_cause'),
            'antecedent_cause' => $this->input->post('antecedent_cause'),
            'underlying_cause' => $this->input->post('underlying_cause'),
            'other_significant_conditions' => $this->input->post('other_significant_conditions'),
            'mccd_form4_issued' => $this->input->post('mccd_form4_issued'),
            'mlc_case' => $this->input->post('mlc_case'),
            'police_informed' => $this->input->post('police_informed'),
            'crs_registration_no' => $this->input->post('crs_registration_no'),
            'body_handed_over_to' => $this->input->post('body_handed_over_to'),
            'informant_name' => $this->input->post('informant_name'),
            'informant_relation' => $this->input->post('informant_relation'),
            'informant_mobile' => $this->input->post('informant_mobile'),
            'remarks' => $this->input->post('remarks')
        );

        $result = $this->death_register_model->create_from_ipd($ipd, $post_data, $this->rbac->get_uid());
    ...
    function download_discharge_sheet($ipd_id = null)
    {
        if (empty($ipd_id) || !ctype_digit((string) $ipd_id)) {
            show_error('IPD record not found.', 404);
            return;
        }

        $this->load->model('patient/ipd_model');
        $data = $this->ipd_model->get_discharge_sheet_data($ipd_id);
        ...
        $status = strtolower(trim(isset($patient['status']) ? (string) $patient['status'] : ''));
        $discharge_date = trim(isset($patient['DoDischarge']) ? (string) $patient['DoDischarge'] : '');
        $has_discharge_date = $discharge_date !== '' && $discharge_date !== '0000-00-00';

        if ($status !== 'discharge' && $status !== 'discharged' && !$has_discharge_date) {
            show_error('Discharge sheet is available only after discharge.', 403);
            return;
        }

        $this->load->helper('discharge_sheet');
        $settings = $this->application_settings();
        $logo = !empty($settings['logo_img'])
            ? '<img src="data:image/png;base64,' . base64_encode($settings['logo_img']) . '" width="58" height="58" />'
            : '';
        $hospital_name = !empty($settings['college_name']) ? html_escape($settings['college_name']) : 'Agastya Ayurveda Medical College and Hospital';
        $hospital_place = !empty($settings['place']) ? html_escape($settings['place']) : 'Koppal';
        $generated_on = date('d-m-Y H:i');
        $header = '<table width="100%" style="border-bottom:1px solid #555555;font-family:sans-serif;">'
            . '<tr><td width="15%" align="center">' . $logo . '</td>'
            . '<td width="70%" align="center"><div style="font-size:15pt;font-weight:bold;">' . $hospital_name . '</div>'
            . '<div style="font-size:9pt;">' . $hospital_place . '</div>'
            . '<div style="font-size:13pt;font-weight:bold;margin-top:3px;">IPD DISCHARGE SUMMARY</div></td>'
            . '<td width="15%" align="right" style="font-size:7.5pt;vertical-align:bottom;">Generated On<br>' . $generated_on . '</td></tr></table>';
        $footer = '<table width="100%" style="border-top:1px solid #777777;font-family:sans-serif;font-size:7.5pt;color:#555555;">'
            . '<tr><td width="35%">Generated by VHMS</td><td width="30%" align="center">Page {PAGENO} of {nbpg}</td>'
            . '<td width="35%" align="right">Powered by Dharani Tech Solutions</td></tr></table>';

        $html = $this->load->view('patient/patient/discharge_sheet_pdf', $data, true);
        generate_pdf($html, 'P', null, 'ipd_discharge_sheet_' . $ipd_id, true, true, 'D', array(
            'header_html' => $header,
            'footer_html' => $footer,
            'margin_top' => 34,
            'margin_bottom' => 14,
