...
/**
 * Description         : Rest web service functions
 * Created             : 14-Nov-2017
 * Author              : Shivaraj B
...
class REST_Server extends SHV_Controller
{
        ...
        switch ($this->request->method) {
            case 'get':
                ...
                if (!empty($query_string)) {
                    ...
                    $get = []; // Default to an empty array to avoid errors
                }
    ...
    private function _detect_format()
    {
        ...
        $query_string = $this->input->server('QUERY_STRING') ?? ''; // Ensure it's a string

        if (!empty($query_string) && preg_match('/format=([^&]+)/', $query_string, $matches)) {
    ...
    private function _prepare_basic_auth()
    {
        ...
        // mod_php
        //code updated by Shivaraj B . added base64_decode
        ...
        elseif ($this->input->server('HTTP_AUTHENTICATION')) {
            if (strpos(strtolower($this->input->server('HTTP_AUTHENTICATION')), 'basic') === 0) {
                list($username, $password) = explode(':', substr($this->input->server('HTTP_AUTHORIZATION'), 6));
            }
        }
