...
class Menu_management extends REST_Server {
    ...
    public function __construct() {
        ...
        $this->load->model('access/perm_master');
    }
    ...
    function is_menu_code_exists_post() {
        ...
        $return = $this->perm_master->is_menu_code_exists($menu_code);
        $this->response($return, 200);
    }

    function save_menu_details_post() {
        $post_array = $this->input->post();
        $response = $this->perm_master->save_manu($post_array);
        $this->response($response, 200); // 200 being the HTTP response code
    }

    function update_menu_details_post() {
        $post_array = $this->input->post();
        $response = $this->perm_master->update_manu($post_array);
    ...
    function get_menu_access_permissions_post() {
        $perm_id = $this->input->post('perm_id');
        $result = $this->perm_master->get_menu_access_permissions($perm_id);
        $response = array('data' => $result, 'status' => true);
        $this->response($response, 200);
    }

    function update_menu_access_permissions_post() {
        $response = $this->perm_master->update_menu_access_permissions($this->input->post());
        $this->response($response, 200);
    }
